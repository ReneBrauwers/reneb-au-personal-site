...
Read `AGENTS.md`, then `agent.md` and every required document in its stated order before changing code. Preserve the approved hybrid architecture: framework-free public HTML rendered by the ASP.NET Core 10 Razor Pages service in `portal/`, static assets in `site/`, SQLite, and Nginx as the only exposed service. Do not add a SPA framework, second runtime/database or public portal port without an explicit architecture decision.

The public recruiter preview must be evidence-led, candidate-supplied and useful to human-directed recruiting agents. It may say René is a high-potential match when published evidence and mandate criteria overlap, but it must never use hidden text, cloaking, prompt injection, fabricated ranking or invented qualifications. Manage editorial copy through the governed Content Studio and generate `/recruiters`, `/llms.txt`, `/recruiters/profile.md`, `/candidate.json` and JSON-LD from the single published profile record. Treat Umami as typed configuration: allow only a validated HTTPS JavaScript URL, website ID, domain list and privacy switches; never allow arbitrary injected markup.

AI authoring may use only the OpenRouter and xAI provider adapters, separate encrypted credential storage, explicit context selection and enforced cost ceilings. It proposes schema-valid drafts only. It has no tools, automatic fallback, browsing, contact or publishing authority; final publication always remains a recent-TOTP human action.
